...
export function Comunicados({user}) {
  ...
  const renderSection = () => {
    switch (currentSection) {
      case 'entrada':
        return <Entrada user={user}/>;
      case 'redactar':
        return <Redactar />;
      case 'enviados':
        return <Enviados user={user}/>;
      default:
        return <Entrada user={user} />;
    }
  };
          ...
          <div className='grid w-full laptop:mb-0 laptop:flex  mt-4 mb-[15vh] laptop:grid-cols-1'>
            <div className='w-1/4 max-sm:w-full max-sm:rounded-3xl max-sm:mb-4  max-md:rounded-3xl max-md:mb-4 max-laptop:w-[100%] max-laptop:rounded-3xl max-laptop:mb-4 bg-white p-5 rounded-l-3xl'>
              <div className='flex items-center space-x-2 border-b pb-5 mb-5 '>
                <span className='font-medium text-zinc-700 text-[clamp(0.8rem,_1rem,_1.3rem)]'>{user.email}</span>
              </div>
              <div className='space-y-2'>
                ...
                <div>
                  <button className='w-full text-left p-2 ml-{2rem} hover:text-blue-500 rounded hover:bg-primary' onClick={() => setCurrentSection('redactar')}>Redactar</button>
                </div>
                <div>
